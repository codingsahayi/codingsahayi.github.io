...
# We want to replace everything from "        // --- ROUTING PHASE ---" up to "        if (NativeTools._fileBackups.Count > 0"

process_replacement = """        // --- ROUTING PHASE ---
            ...
            try
            {
                ...
                if (completion.FinishReason == ChatFinishReason.ToolCalls)
                {
                    ...
                    foreach (var toolCall in completion.ToolCalls)
                    {
                        ...
                        try
                        {
                            ...
                            while (retryCount < 2)
                            {
                                var args = System.Text.Json.JsonSerializer.Deserialize<System.Text.Json.JsonElement>(argsStr);
                                ...
                                if (toolIsModifying && success)
                                {
                                    ...
                                    if (toolCall.FunctionName == "batch_patch_file") {
                                        if (args.TryGetProperty("patches", out var pArr) && pArr.ValueKind == System.Text.Json.JsonValueKind.Array && pArr.GetArrayLength() > 0)
                ...
                else
                {
                    ...
                    if (finalResponse.TrimStart().StartsWith("{") && finalResponse.Contains("\\"name\\"") && finalResponse.Contains("\\"parameters\\""))
                    {
                        try
                        {
                            using var doc = System.Text.Json.JsonDocument.Parse(finalResponse);
...
eval_replacement = """    private async Task<bool> EvaluatePatchAsync(string filePath, string proposedPatch)
    {
        string syntaxStatus = WorkspaceCodeAnalysisService.VerifySyntax(filePath);
...
for i in range(len(lines)):
    line = lines[i]
    if line.startswith("        // --- ROUTING PHASE ---"):
        skip = True
        new_lines.append(process_replacement + "\n")
        continue
    
    if skip and line.startswith("        if (NativeTools._fileBackups.Count > 0"):
        skip = False
    
    if skip:
        continue
    
    if "private async Task<bool> EvaluatePatchAsync" in line:
        skip = True
        new_lines.append(eval_replacement + "\n")
        continue
        
    if skip and "private async Task<(string result, bool success)> ExecuteToolAsync" in line:
        skip = False
